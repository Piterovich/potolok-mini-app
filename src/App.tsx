...
const RoomCanvas = ({ room, updateRoom, options, theme }) => {
  ...
  const syncElementsToInputs = (newEls) => {
      ...
      let trackLen = 0;
      newEls.filter(e => e.type === 'track').forEach(tr => {
          for(let i=1; i<tr.points.length; i++) {
              trackLen += Math.sqrt((tr.points[i].x - tr.points[i-1].x)**2 + (tr.points[i].y - tr.points[i-1].y)**2);
          }
      });
  ...
  useEffect(() => {
    ...
    if (mode === 'track' && activeTrackPts.length > 0) {
        ...
        activeTrackPts.forEach(p => {
            let sp = toScreen(p);
            ctx.beginPath(); ctx.arc(sp.x, sp.y, 5, 0, 2*Math.PI); ctx.fillStyle = t.warning; ctx.fill();
        });
    }
    ...
    for(let i = 0; i < screenPts.length; i++) {
       ...
       if (mode === 'remove') ctx.fillStyle = t.danger;
       else if (mode === 'add_diag' && selectedDiagPt === i) ctx.fillStyle = t.warning; 
       else ctx.fillStyle = draggingIdx === i ? t.danger : t.card;
       ctx.fill(); ctx.lineWidth = 3; ctx.strokeStyle = (mode === 'add_diag' && selectedDiagPt === i) ? t.warning : t.danger; ctx.stroke();
       const label = String.fromCharCode(65 + i); 
       ctx.fillStyle = t.text; ctx.font = '900 16px system-ui'; ctx.fillText(label, sp.x + 18, sp.y - 12);
    }

    ctx.fillStyle = t.text; ctx.font = '900 16px system-ui'; ctx.textAlign = 'left';
  ...
  return (
    <div style={{ position: 'relative', textAlign: 'center', marginBottom: '15px' }}>
      ...
      <div style={{ height: '24px', marginBottom: '8px', fontWeight: '800', fontSize: '13px', 
        color: viewMode === '3d' ? t.danger : (['add', 'spot', 'chand', 'track', 'pipe'].includes(mode) ? t.success : (mode === 'remove' ? t.danger : t.subText)) }}>
          {getHelperText()}
      </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '16px' }}>
            {room.logicalPts?.map((p, i) => {
                ...
                <div key={name} style={{ display: 'flex', alignItems: 'center', background: t.card, padding: '8px 12px', borderRadius: '10px', border: `1px solid ${t.border}`, boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                    ...
                    <input type="number" value={displayVal} onChange={(e) => updateRoom(room.id, 'manualWalls', {...(room.manualWalls || {}), [name]: e.target.value})} style={{ width: '55px', border: 'none', background: 'transparent', outline: 'none', fontWeight: 'bold', fontSize: '15px', color: t.text }}/>
                </div>
                )
            })}
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {room.activeDiags?.map((diagName, index) => {
                ...
                <div key={index} style={{ display: 'flex', alignItems: 'center', background: t.card, padding: '6px 8px', borderRadius: '10px', border: `1px solid ${t.border}`, boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                    <select translate="no" className="notranslate" value={diagName} onChange={(e) => { const newD = [...room.activeDiags]; newD[index] = e.target.value; updateRoom(room.id, 'activeDiags', newD); }} style={{ border: 'none', outline: 'none', fontWeight: '800', color: t.warning, background: 'transparent', fontSize: '14px', marginRight: '4px' }}>
...
function App() {
  ...
  const options = {
    ...
    // ⚠️ id ОБЯЗАТЕЛЬНО совпадают с ключами прайса на бэкенде — иначе цена не подтянется
    profiles: [
      { id: 'профиль_м',                          name: 'ПВХ стеновой' },
      { id: 'профиль_алюминиевый_стеновой_мп',    name: 'Алюминиевый стеновой' },
      { id: 'профиль_потолочный_алюминий_мп',     name: 'Потолочный (алюминий)' },
      { id: 'профиль_теневой_6мм_мп',             name: 'Теневой 6 мм' },
      { id: 'профиль_теневой_10мм_мп',            name: 'Теневой 10 мм' },
    ...
    cornices: [
      ...
      { id: 'карниз_Q7_мп',                      name: 'Карниз Q7' },
      { id: 'карниз_Q10_мп',                     name: 'Карниз Q10' },
      { id: 'карниз_Q11_мп',                     name: 'Карниз Q11' },
      { id: 'карниз_K4_мп',                      name: 'Карниз K4' },
      { id: 'карниз_K5_мп',                      name: 'Карниз K5' },
      { id: 'карниз_ПК5_мп',                     name: 'Карниз ПК5' },
      { id: 'ниша_под_штору_с_заворотом_мп',     name: 'Ниша для шторы (с заворотом)' },
      { id: 'ниша_под_штору_обрыв_мп',           name: 'Ниша для шторы (обрыв)' },
    ]
  };

  // ⚠️ Эти цены — ТОЛЬКО для локального превью «Предварительно».
  // Реальный расчёт всегда идёт на бэкенде по индивидуальному прайсу пользователя.
  // Цифры здесь — типичный среднерыночный ориентир для замерщика без своего прайса.
  const [prices] = useState({
    canvas: { 'полотно_м2': 330, 'msd_premium_320_м2': 350, 'черный_матовый_м2': 400 },
    profile: {
      'профиль_м':                          80,
      'профиль_алюминиевый_стеновой_мп':    100,
      'профиль_потолочный_алюминий_мп':     100,
      'профиль_теневой_6мм_мп':             350,
      'профиль_теневой_10мм_мп':            400,
    ...
    return (
      <div style={{ animation: 'fadeIn 0.3s ease-in', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', marginBottom: '20px', padding: '0 8px', boxSizing: 'border-box' }}>
            ...
            <button onClick={() => { triggerHaptic(); setTheme(theme === 'light' ? 'dark' : 'light'); }} style={{ background: ts.card, border: `1px solid ${ts.border}`, width: '44px', height: '44px', borderRadius: '14px', fontSize: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', cursor: 'pointer' }}>
                {theme === 'light' ? '🌙' : '☀️'}
            </button>
        </div>
            ...
            <div style={{ display: expandedRoomId === room.id ? 'block' : 'none', animation: 'slideDown 0.2s ease-out' }}>
                <div>
                  <div style={styles.subHeader} onClick={() => { triggerHaptic(); setExpandedSubSec(expandedSubSec === 'geom' ? null : 'geom')}}>
                    <span style={{ fontWeight: '700', fontSize: '16px', color: ts.text }}>{t('geom')}</span>
                ...
                <div>
                  <div style={styles.subHeader} onClick={() => { triggerHaptic(); setExpandedSubSec(expandedSubSec === 'mat' ? null : 'mat')}}>
                    <span style={{ fontWeight: '700', fontSize: '16px', color: ts.text }}>{t('materials')}</span>
                ...
                <div>
                  <div style={styles.subHeader} onClick={() => { triggerHaptic(); setExpandedSubSec(expandedSubSec === 'light' ? null : 'light')}}>
                    <span style={{ fontWeight: '700', fontSize: '16px', color: ts.text }}>{t('lighting')}</span>
                    <span style={{ color: ts.subText, fontSize: '14px' }}>{expandedSubSec === 'light' ? '▲' : '▼'}</span>
                  </div>
                ...
                <div>
                  <div style={styles.subHeader} onClick={() => { triggerHaptic(); setExpandedSubSec(expandedSubSec === 'corniceSec' ? null : 'corniceSec')}}>
                    <span style={{ fontWeight: '700', fontSize: '16px', color: ts.text }}>{t('corniceSec')}</span>
                ...
                <div>
                  <div style={styles.subHeader} onClick={() => { triggerHaptic(); setExpandedSubSec(expandedSubSec === 'dops' ? null : 'dops')}}>
                    <span style={{ fontWeight: '700', fontSize: '16px', color: ts.text }}>{t('dops')}</span>
                    <span style={{ color: ts.subText, fontSize: '14px' }}>{expandedSubSec === 'dops' ? '▲' : '▼'}</span>
                  </div>
